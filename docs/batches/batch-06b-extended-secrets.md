# Batch 6b — More secret categories (AI providers, webhooks, more vendors)

## Where we are — 2026-09-03 (after first scan #17)

**Is this batch done? PARTLY — 10 out of 22 working, 12 need small tweaks**

**✅ Working (10 checks firing):**
1. `jwt-weak-signing-secret` — big win! Scanner cracked our JWT (signed with the weak secret "secret") using a 103,781-entry wordlist
...
5. `database-provider-keys-in-js-bundle` — **9 findings** (Neon Postgres + MySQL + Mongo connection strings, plus our SSR payload DB URLs as bonus)
...
**🟡 Open fixes (12 checks) — all tracked in [testbed-fixes-backlog.md](../testbed-fixes-backlog.md) under Fix N:**
- `agentmail` — `agm_live_...` format doesn't match
- `auth-provider` — Auth0 + WorkOS don't match; Clerk got detected as Stripe (small false positive we accept)
...
- `generic-public-env` — needs real `NEXT_PUBLIC_X="..."` variable declarations, not strings in JSON
- `netlify-pat` — `nfp_...` doesn't match the check regex
...
- `sensitive-data-in-initial-payload` — our DB URLs got captured by the database check; needs a proper SSR-payload-shaped hash
...
- No js-exception-regression — that's correct (no new console errors since prior scan)

---

- **Category:** More secret categories (V6 bundle categories)
- **Checks tested:** 22 (all of Batch 6b)
...
- LLM providers besides OpenAI/Anthropic (Groq, Together, Replicate, Perplexity)
...
- `jwt-weak-signing-secret` — our authtoken cookie is a JWT signed with the weak secret "secret" (BugBuzzer's dictionary attack cracks it)
...
**All values are FAKE** — verified by BugBuzzer's liveness probes (real providers reject them).
...
- **[`middleware.ts`](../../middleware.ts)** — updated `authtoken` cookie to a JWT signed with the weak secret "secret" (for `jwt-weak-signing-secret`)
...
For each finding kind, the report tells the customer:

- **Any secret in JS bundle** → move to a server-only environment variable (no `NEXT_PUBLIC_/VITE_` prefix), route calls through a server-side proxy
- **Weak JWT signing secret** → rotate to a cryptographically strong random secret (`openssl rand -base64 32`)
- **Sensitive data in initial payload** → never render password hashes / DB connection strings in server HTML — strip them before serialization
- **Public env with secret-shaped value** → audit every `NEXT_PUBLIC_*` variable, move any that look like secrets to server-only
...
- Did the `authtoken` cookie in middleware get changed away from the weak-signed JWT?
- Did a Next.js update change how string literals get tree-shaken?
