...
|---|---|---|---|---|---|---|---|
| 2026-08-24 04:36 | beta | 1 | 7 (Batch 1) | 18 detections for Batch 1 (multi-source) + 11 real hygiene findings = 29 total | **7 of 7 for Batch 1** | 0 for Batch 1 | Scan ref BB-20260824-**5B681D**. ✅ **BATCH 1 COMPLETE.** All 7 checks fired with correct evidence, severity, confidence, and rich context (redaction, entropy, fingerprint, live probe results). Bonus: multi-source detection (fetched chunk + inline JSON) + live credential probing against provider APIs (all correctly show "already rotated/revoked" for our fake keys). Minor cosmetic bug spotted: check summary count overstates findings count by 1 per source with `sourceUrl=null` (dedup collision on inline vs pageHtml). See [scan-issues/2026-08-24-scan-04.md](./scan-issues/2026-08-24-scan-04.md). |
